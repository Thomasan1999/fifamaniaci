    ...
    <div class="tab-container">
        <template v-for="[tabId, tab] in tabs">
            ...
            <router-link v-else :key="tabId" class="tab" :to="{name: tabsNames[tabId], params: $store.getters.params}">
                ...
                <unseen-box-container v-if="unseen" :count="unseen[tabId]"/>
...
<script lang="ts">
    ...
    export default class TabContainer extends mixins(MainMixin)
    {
        ...
        @Prop({
            ...
            {
                // @ts-ignore
                return this.$store.state[this.tableName]?.unseen[this.$store.state.categories.active] || {};
            },
            type: Object
        }) public readonly unseen!: {general: number, personal: number};
        @Prop({
            default: false,
            type: Boolean
