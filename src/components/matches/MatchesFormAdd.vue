<template>
    <tr class="matches-form-add" :class="new $String(type.name).caseTrainTo().toString()">
        <td v-for="(element, elementName) in elements" :key="elementName" :class="tdClassGet({columnName: elementName})"
            ...
            <div v-else-if="element.tag === `input`" v-show="element.input !== `checkbox` || element.visibility"
                ...
                <input v-else ref="elements" :autocomplete="element.autocomplete" :checked="element.value"
                       ...
                       :placeholder="(text[elementName] && text[elementName].localization) || text[elementName]"
        ...
        <template v-for="columnName in [`submit`, `delete`]">
            <td :class="tdClassGet({columnName})" :key="columnName" v-html="columns[columnName].html" @click.stop="clickOn" @mousedown.prevent/>
        </template>
    </tr>
</template>
    ...
    export default class MatchesFormAdd extends mixins(MainMixin, MatchesMixin, FormMixin, UserDropdownContainerMixin)
    {
        ...
        public submitOn(): void
        {
            ...
            this.$store.dispatch(`tableUpdate`, {
                ...
                res: {
                    body: {
                        [Object.keys(this.$store.state.matches.value[this.categoriesActive]).length + 1000]: {
                            ...this.formData,
                            overtime: this.formData.overitime ? true : null
                        }
                    },
                    ok: true
                } as unknown as any,
        ...
        public tdClassGet({columnName}: { columnName: string }): string
        {
            return new this.$String(columnName).caseTrainTo().toString();
        }
