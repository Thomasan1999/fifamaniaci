<template>
    <section>
        <loading-container :condition="dataLoaded">
            ...
            <tab-container :tabs="tabs" :tabs-active="tabsActive" :tabs-names="tabsNames" :text="text"/>
            <router-view class="subsection"/>
        </loading-container>
    </section>
</template>
...
<script lang="ts">
    ...
    export default class League extends mixins(MainMixin, SectionMixin, TabContainerContainerMixin)
    {
        ...
        {
            const {commit, dispatch, getters, state} = this.$store;
