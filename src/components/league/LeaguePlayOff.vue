<template>
    <div>
        <loading-container :condition="dataLoaded">
            <div v-if="rounds.length" class="bracket" :style="bracketStyleGet()">
            ...
            <div v-else class="text-container">
                <p v-html="new $String(text.placeholder).htmlParse()"/>
            </div>
        </loading-container>
    </div>
</template>
    ...
    export default class LeaguePlayOff extends mixins(MainMixin, LeagueSectionMixin)
    {
        ...
        public get columns()
        {
            ...
            return {
                ...
                ...Array((this.$store.getters.leagueSeason.playOffWinsLimit * 2) - 1).fill(null).reduce((a, _, legIndex) =>
                {
        ...
        public get rounds()
        {
            ...
            if (leagueTableRecords.length < 2)
            {
                return [];
            }
