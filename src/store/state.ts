...
const stateExportable: {
    ...
    leaguePrizes: Merge<StoreTable<LeaguePrizeValue>, {
        nestedProps: string[],
        unseen: { [s: string]: { [s: string]: { [s: string]: Unseen } } }
    }>,
    leagueRegistrations: Merge<StoreTable<LeagueRegistrationValue>, {
        ...
        unseen: { [s: string]: { [s: string]: boolean } },
    ...
    leagueRegistrations: {
        ...
        rowsGet()
        {
            ...
            }).map(([leagueRegistrationId, leagueRegistration]) =>
            {
                ...
                return [leagueRegistrationId, {...leagueRegistration, divisionExpected: divisionExpectedValue, ...(preStart && typeof leagueRegistration.rating === `undefined` && {rating})}];
            }));
        },
    ...
    leagueTableRecords: {
        ...
        unseen: {},
        unseenPersonalCondition(leagueTableRecord)
        {
            return leagueTableRecord.userId === store.state.userLogged.id;
        },
        value: {}
    },
    leagueTabs: {
    ...
    matches: {
        ...
        sides: [`home`, `away`],
        unseen: {},
        unseenPersonalCondition(match)
        {
            return this.sides.map((side) =>
            {
                return match[`${side}Id`];
            }).includes(store.state.userLogged.id);
        },
        value: {}
    },
    matchesTypes: {
    ...
    messages: {
        ...
        tabs: true,
        unseen: {},
        unseenPersonalCondition(message)
        {
            return Boolean(message.addresseeId);
        },
        value: {}
    },
    messagesTabs: {
    ...
    players: {
        ...
        finished: {},
        unseen: {},
        unseenPersonalCondition(player)
        {
            return player.userId === store.state.userLogged.id;
        },
        value: {}
    },
    profileUsername: ``,
    ...
    unseenTypes: [`general`, `personal`],
    userLogged: {
        id: parseInt(VueCookies.get(`id`) || `0`),
        email: ``,
        fbLink: ``,
        loggedIn: Boolean(VueCookies.get(`token`)),
        money: 0,
